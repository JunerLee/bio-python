...
乳腺癌是全球女性最常见的恶性肿瘤之一。及时准确地判断肿瘤的**良恶性**，对临床治疗方案的选择至关重要：

- **早期发现**：良性肿瘤通常无需化疗，恶性肿瘤越早确诊，治疗效果越好
- **减少漏诊**：假阴性（将恶性误判为良性）可能延误治疗，后果严重
- **辅助决策**：机器学习模型可以作为病理医生的"第二双眼睛"

### 我们要做什么？

通过**细胞核的形态学测量特征**（来自细针穿刺活检的数字化显微图像），
...
**交叉验证中的数据泄漏陷阱：Pipeline**

在交叉验证中，一个常见的错误是**先对整个训练集做标准化，再用标准化后的数据做交叉验证**。这会导致每一折的验证数据参与了 scaler 的 `fit`，造成数据泄漏。

正确做法是使用 `Pipeline` 将标准化和模型封装在一起，确保每一折内部独立完成标准化：
...
X_scaled = scaler.fit_transform(X_train)          # 整个训练集参与 fit
scores = cross_val_score(model, X_scaled, y, cv=5) # 验证折也被 scale 过了

# ✅ 正确：Pipeline 确保每一折独立标准化
pipeline = Pipeline([
    ("scaler", StandardScaler()),
    ("model", LogisticRegression()),
])
scores = cross_val_score(pipeline, X_train, y, cv=5)  # 每折内部独立 fit
```
