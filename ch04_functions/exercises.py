...
# 为了聚焦函数练习，本题做了一个教学简化：
# 直接用 DNA 三联体（含 T）查表翻译，省略 DNA → mRNA 的转录步骤。
# 实际生物学翻译发生在 mRNA 上，也就是把这里的 T 对应为 U。
#
# 【任务】
...
def translate(dna_seq):
    ...
    #
    # 提示步骤：
    # 1. 用 for i in range(0, len(dna_seq), 3) 遍历序列
    # 2. 取出密码子：codon = dna_seq[i:i+3]
    # 3. 查密码子表：amino_acid = CODON_TABLE[codon]
    # 4. 如果是终止密码子（"*"），用 break 停止
    # 5. 否则把氨基酸追加到 protein 字符串中
    #
...
def similarity(seq1, seq2):
    ...
    #
    # 提示步骤：
    # 1. 用 for base1, base2 in zip(seq1, seq2) 遍历
    # 2. 如果 base1 == base2，match_count 加 1
    # 3. 最后返回 round(match_count / len(seq1) * 100, 2)
    #
